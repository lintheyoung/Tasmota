...
# 网关配置（新创建的设备）
THING_NAME = "IoT-Gateway-000003"
GATEWAY_DEVICE_ID = "aaf69e41-b7c5-433d-bc5d-30bdc7960edf"
...
esp32_gateway_simulator.py:
```
"""
ESP32 网关模拟器 - 支持低功耗子设备模式
- 网关：7x24 小时在线，MQTT 连接始终保持
- 子设备：低功耗模式，定时唤醒连接网关
"""
import json
import time
...
class LowPowerDevice:
    ...
    def __init__(self, device_config):
        self.shadow_name = device_config['shadow_name']
        self.name = device_config['name']
        self.type = device_config['type']
        self.data_generator = device_config['data_generator']
        self.interval = device_config['interval']
        self.wakeup_interval = device_config.get('wakeup_interval', 3600)
        self.low_power = device_config.get('low_power', False)

        # 设备状态
        self.is_awake = False
        self.last_wakeup_time = 0
        self.battery = 100  # 电池电量
        self.local_state = {}  # 本地状态（模拟设备内存）

    def generate_sensor_data(self):
        """根据设备类型生成模拟数据"""
        if self.data_generator == 'temperature':
            return {
        ...
        elif self.data_generator == 'humidity':
            return {
        ...
        elif self.data_generator == 'relay':
            # 如果有本地状态，使用本地状态，否则随机生成
            if self.local_state:
                return {
                    **self.local_state,
                    'battery': self.battery,
            ...
            return {
                'channel_1': random.choice([True, False]),
                'channel_2': random.choice([True, False]),
                'channel_3': random.choice([True, False]),
                'channel_4': random.choice([True, False]),
                'battery': self.battery,
                'timestamp': datetime.now().isoformat()
            }
        else:
    ...
    def wakeup(self):
        ...
        # ✅ 常电供电设备电池保持 100%
        if self.low_power:
            self.battery = max(0, self.battery - 0.1)  # 低功耗模式：唤醒消耗 0.1% 电量
        else:
            self.battery = 100.0  # 常电供电：保持满电
        print(f"\n🔔 [{self.name}] 设备唤醒 (电量: {self.battery:.1f}%)")

    def sleep(self):
        """设备休眠"""
        self.is_awake = False
        print(f"😴 [{self.name}] 进入休眠 (下次唤醒: {self.wakeup_interval}秒后)")

    def execute_command(self, command):
        """执行命令"""
        print(f"   ⚙️  [{self.name}] 执行命令: {command}")
...
class ESP32Gateway:
    ...
    def __init__(self):
        ...
        self.devices = [LowPowerDevice(cfg) for cfg in config.CHILD_DEVICES]
        self.mqtt_connection = None
        self.is_connected = False

        # 命令缓存队列（模拟网关内存）
        self.command_queue = {device.shadow_name: deque() for device in self.devices}

        print(f"\n🚀 ESP32 网关模拟器初始化")
        print(f"   Thing Name: {self.thing_name}")
        print(f"   IoT Endpoint: {self.endpoint}")
        print(f"   子设备数量: {len(self.devices)}")

    def connect_to_aws_iot(self):
        """连接到 AWS IoT Core（使用 X.509 证书认证）"""
        print(f"\n🔌 正在连接到 AWS IoT...")
        ...
        self.mqtt_connection = mqtt_connection_builder.mtls_from_path(
            ...
            cert_filepath=config.CERTIFICATE_PATH,
            pri_key_filepath=config.PRIVATE_KEY_PATH,
            ca_filepath=config.ROOT_CA_PATH,
            client_id=self.thing_name,
            clean_session=False,
            keep_alive_secs=30,  # ✅ 优化：从 60s 改为 30s，更快检测断线
        ...
        print(f"✅ 已连接到 AWS IoT Core!\n")

    def _on_connection_interrupted(self, connection, error, **kwargs):
        print(f"⚠️ 连接中断: {error}")
        self.is_connected = False

    def _on_connection_resumed(self, connection, return_code, session_present, **kwargs):
        print(f"✅ 连接恢复: return_code={return_code}")
        self.is_connected = True

    def subscribe_to_shadow_deltas(self):
        """订阅所有子设备的 Shadow delta（接收云端命令）"""
        print(f"📡 订阅 Shadow Delta（接收云端命令）...")

        for device in self.devices:
            topic = f"$aws/things/{self.thing_name}/shadow/name/{device.shadow_name}/update/delta"
            print(f"   订阅: {topic}")

            subscribe_future, _ = self.mqtt_connection.subscribe(
                topic=topic,
                qos=mqtt.QoS.AT_LEAST_ONCE,
                callback=lambda topic, payload, device=device: self._on_shadow_delta(device, topic, payload)
            )
            subscribe_future.result()

        print("✅ Shadow Delta 订阅完成\n")

    def _on_shadow_delta(self, device, topic, payload, **kwargs):
        """处理 Shadow delta（云端命令）- 缓存到队列"""
        try:
            ...
            if 'state' in message:
                ...
                req_id = command.get('_reqId', 'unknown')
                priority = command.get('_priority', 'normal')

                # 将命令加入缓存队列
                self.command_queue[device.shadow_name].append({
                    'command': command,
                    'reqId': req_id,
                    'priority': priority,
                    'timestamp': time.time()
                ...
                print(f"   📦 命令已加入缓存队列")
                print(f"   队列长度: {len(self.command_queue[device.shadow_name])}")

                # 如果设备当前在线，立即执行
                if device.is_awake:
                    print(f"   ✅ 设备在线，立即执行命令")
                    self._execute_queued_commands(device)
                else:
                    print(f"   😴 设备休眠中，等待下次唤醒")

        except Exception as e:
            print(f"   ❌ 处理命令失败: {e}")

    def _execute_queued_commands(self, device):
        """执行队列中的命令"""
        queue = self.command_queue[device.shadow_name]

        if not queue:
            print(f"   ✅ 无待执行命令")
            return

        print(f"\n📦 开始执行缓存命令 (共 {len(queue)} 条)")

        # 按优先级排序
        sorted_commands = sorted(queue, key=lambda x: 0 if x['priority'] == 'high' else 1)
        ...
        # 执行合并后的命令
        if merged_state:
            print(f"   ⚙️  合并命令执行: {merged_state}")
            device.execute_command(merged_state)

            # 构造响应状态
            response_state = device.generate_sensor_data()
            response_state.update(merged_state)
            response_state['_reqIds'] = req_ids

            # 上报到 Shadow
            self.update_shadow_reported(device, response_state)

            # 清空队列
            queue.clear()
            print(f"   ✅ 命令执行完成，队列已清空")

    def update_shadow_reported(self, device, state):
        """更新 Shadow reported 状态（上报设备当前状态）"""
        topic = f"$aws/things/{self.thing_name}/shadow/name/{device.shadow_name}/update"
        ...
        print(f"   📤 上报设备状态: {device.shadow_name}")

        # 异步发布，不阻塞（避免在 Delta 回调中死锁）
    ...
    def start_low_power_device_simulator(self):
        """启动子设备模拟器（支持低功耗和常电供电）"""
        print(f"🔋 启动子设备模拟器...")

        for device in self.devices:
            if device.low_power:
                print(f"   ✅ {device.name} (低功耗模式，唤醒周期: {device.wakeup_interval}秒)")
            else:
                print(f"   🔌 {device.name} (常电供电，保持在线，上报间隔: {device.interval}秒)")

            thread = threading.Thread(
                target=self._low_power_device_loop,
                args=(device,),
                daemon=True
            )
            thread.start()

    def _publish_heartbeat_for_child_wakeup(self):
        """子设备唤醒/休眠时立即发送心跳"""
        topic = f"devices/{self.thing_name}/status"
        payload = {
            ...
            'child_devices': [
                {
                    'shadow_name': device.shadow_name,
                    'name': device.name,
                    'type': device.type,
                    'is_awake': device.is_awake,
                    'battery': round(device.battery, 1),
                    'last_wakeup': device.last_wakeup_time,
    ...
    def _low_power_device_loop(self, device):
        """低功耗设备循环"""
        while self.is_connected:
            try:
                ...
                # ✅ 检查设备是否为常电供电（保持在线）
                if not device.low_power or device.wakeup_interval is None:
                    # 常电供电模式：保持在线，持续上报数据
                    if not device.is_awake:
                        device.wakeup()
                        print(f"🔌 [{device.name}] 常电供电模式 - 保持在线")
                    ...
                    self.update_shadow_reported(device, sensor_data)
                    ...
                    # 低功耗模式：定时唤醒
                    # 检查是否到了唤醒时间
                    if current_time - device.last_wakeup_time >= device.wakeup_interval:
                        # 设备唤醒
                        device.wakeup()

                        # ✅ 立即发送心跳通知唤醒状态
                        self._publish_heartbeat_for_child_wakeup()
                ...
                print(f"❌ [{device.name}] 循环错误: {e}")
                time.sleep(5)

    def publish_gateway_heartbeat(self):
        """网关心跳（上报网关自身状态到 Classic Shadow）"""
        # ⚡ 启动延迟：等待连接完全稳定
        time.sleep(3)
        ...
        while self.is_connected:
            try:
                ...
                # 统计子设备状态
                awake_count = sum(1 for d in self.devices if d.is_awake)
                queue_counts = {k: len(v) for k, v in self.command_queue.items()}
                    ...
                    'state': {
                        'reported': {
                            ...
                            'child_devices': {
                                ...
                                'awake': awake_count,
                                'sleeping': len(self.devices) - awake_count
                            },
    ...
    def publish_device_status(self):
        """
        ✅ 新增：发布设备状态到 devices/{thing_name}/status Topic
        双路心跳：Shadow（保留）+ 自定义 Topic（新增）
        """
        # ⚡ 启动延迟：等待连接完全稳定后再开始发布
        time.sleep(5)
        ...
        while self.is_connected:
            try:
                # 使用 thing_name 作为标识（Edge Function 会根据 thing_name 查询 device_id）
                topic = f"devices/{self.thing_name}/status"
                ...
                # 发布到 MQTT Topic（使用 QoS 0 减少连接压力）
                self.mqtt_connection.publish(
                    topic=topic,
                    payload=json.dumps(payload),
                    qos=mqtt.QoS.AT_MOST_ONCE
                ...
                print(f"📡 [Status] Published to {topic} (uptime: {payload['gateway_info']['uptime']}s)")

                # 每 20 秒发布一次（必须 < Keep-Alive 30s，否则连接会超时）
                time.sleep(20)

            except Exception as e:
                print(f"❌ [Status] 发布错误: {e}")
                time.sleep(60)

    def run(self):
        """运行网关模拟器"""
        print(f"\n{'='*60}")
        print(f"ESP32 网关模拟器启动")
        print(f"{'='*60}\n")
        ...
        # 订阅 Shadow Delta
        self.subscribe_to_shadow_deltas()

        # 启动低功耗设备模拟器
        self.start_low_power_device_simulator()

        # 启动网关心跳（Shadow）
        heartbeat_thread = threading.Thread(
            target=self.publish_gateway_heartbeat,
            daemon=True
        )
        ...
        status_thread = threading.Thread(
            target=self.publish_device_status,
            daemon=True
        )
        status_thread.start()

        print(f"✨ 网关运行中... (按 Ctrl+C 退出)")
        print(f"   - Keep-Alive: 30秒")
        print(f"   - 心跳间隔: 20秒")
        print(f"   - 双路上报: Shadow + devices/*/status Topic")
        print(f"{'='*60}\n")
...
esp32-gw-demo-01-config.json:
```
{
  "gateway_id": "16d11959-a924-4ff4-99de-1d127321a3ae",
  "thing_name": "esp32-gw-demo-01",
  "iot_endpoint": "a1f8xc5wo59rp8-ats.iot.ap-southeast-1.amazonaws.com",
  "aws_region": "ap-southeast-1",
  "certificate_id": "60048639aa7a8fd46e866cbef5b40a4d04f08d6594f0f241f94c55722865d09f",
  "certificate_arn": "arn:aws:iot:ap-southeast-1:835857361837:cert/60048639aa7a8fd46e866cbef5b40a4d04f08d6594f0f241f94c55722865d09f",
  "created_at": "2025-10-10T15:08:55.895874+00:00"
}
```
